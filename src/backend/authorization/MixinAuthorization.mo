...
mixin (accessControlState : AccessControl.AccessControlState) {
  // Initialize auth - first caller becomes admin (no token required)
  public shared ({ caller }) func _initializeAccessControl() : async () {
    AccessControl.initializeFirstAdmin(accessControlState, caller);
  };

  // Legacy: Initialize with secret token
